...
public class GuiCreateServer {
	...
	private final static String TXT_NAME = "Adresse de binding: ";
	private Text fieldHost = null;
	
	private final static String TXT_CREATE_GAME = "Créer";
	private Button buttonCreateServer = null;
	...
	private String resultHostName = null;
	 ...
	 * @param compositeHost
	 */
	public GuiCreateServer(Composite compositeHost) {
	 ...
	 * @param compositeHost
	 */
	public GuiCreateServer(Display display) {
	...
	protected void updateButtonsStates() {
		...
		buttonCreateServer.setEnabled(
				!fieldHost.getText().trim().isEmpty()
				);
		
		if (buttonCreateServer.isEnabled()) {
			shell.setDefaultButton(buttonCreateServer);
		}
	}
	...
	public void init() {
		...
		// the field
		fieldHost = new Text(compositeHost, SWT.BORDER);
		fieldHost.setLayoutData(new GridData(GridData.FILL_HORIZONTAL | GridData.VERTICAL_ALIGN_BEGINNING | GridData.GRAB_HORIZONTAL));
		try {
			fieldHost.setText(RMIUtils.getLocalHost().getHostAddress());
		} catch (UnknownHostException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		fieldHost.addModifyListener(new ModifyListener() {
		...
		// bottom composite
		{
			...
			buttonCreateServer = new Button(compositeBottom, SWT.PUSH);
			buttonCreateServer.setText(TXT_CREATE_GAME);
			buttonCreateServer.addSelectionListener(new SelectionListener() {
	...
	private void clickOnCreateGame() {
		...
		resultHostName = fieldHost.getText().trim();
